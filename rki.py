...
# control parameters
Kv = 0.7
vmax = 0.055  # maximum velocity of the end effector per axis (m/s)
...
def get_joint_angle(motor_angle_1, motor_angle_2):
    ...
    """
    q1 = motor_angle_1
    q2 = 1 / 2 * np.pi + motor_angle_2 - motor_angle_1  # - might change into + depending on the reference configuration
    return q1, q2
...
def get_J(q1):  # Calculate the Jacobian using the modified jacobian method
    """
...
def get_qdot(motor_angle_1, motor_angle_2, v, ticker_frequency):
    """
    =INPUT= motor_angle_1, motor_angle_2 are motor angles measured, they are converted to the angles of the "virtual"
    joint
    ...
    H0f = np.eye(3)
    # calculate H0 to f (frame f is attached to EE frame, but differs in that it is non-rotating, always level wrt
    # ground)
    H0f[:2, 2] = -pe0
