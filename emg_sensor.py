...
# THE NEW FILTERS - ONLY LET THROUGH THE GREAT STUFF!
...
class EmgSensor(object):
    ...
    def __init__(self):
        self.sample_lists = [[], [], []]
        # we determine this during calibration step, stronger person -> larger factor, begins at 1, then set to max
        # emg value recorded during calibration step
        self.normalization_factors = [1, 1, 1]
        self.window_size = 80
        self.emg_sensor_value = [0, 0, 0]  # initial value of EMG [sensor 1, sensor 2, sensor 3]
        return

    # Combination of multiple filters, Filtering 10Hz high pass to get rid of DC components, then filtering out 50Hz
    # & 100Hz frequency peaks
    ...
    def moving_average(self, data, n=0):
        ...
        # return the average divided by the normalization factor (max emg signal during calibration)
        return cumulative_sum / len(sample_list) / normalization_factor
